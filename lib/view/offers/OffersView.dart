...
class OffersView extends GetView<OffersController> {
  ...
  Widget _buildServiceCard(dynamic service) {
    ...
    return Obx(() {
      final isCreatingOffer = controller.isServiceLoading(serviceId, 'createOffer');
    ...
    return Obx(() {
      ...
      return Row(
        ...
        children: [
          ...
          if (!hasActiveOffer)
            GestureDetector(
              onTap: isCreatingOffer ? null : () => controller.addOfferToService(service),
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                decoration: BoxDecoration(
                  color: isCreatingOffer ? Colors.grey[400] : const Color(0xFFEF4444),
                  borderRadius: BorderRadius.circular(16),
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
          ...
          // Delete Offer button (only if has active offer)
          if (hasActiveOffer && activeOffer != null) ...[
            Obx(() {
              final isDeletingOffer = controller.isOfferLoading(activeOffer.id, 'delete');

              return GestureDetector(
                onTap: isDeletingOffer ? null : () {
                  // تأكد من وجود العرض قبل الحذف
                  if (activeOffer != null) {
